...
`RTCProvider` 当前使用浏览器 `getUserMedia` 与 `RTCPeerConnection` 实现一对一音视频，
Offer、Answer、ICE 通过带课堂权限校验的 `RTC_SIGNAL` 转发。`RTCProviderAdapter`
仍保留 `join`、`leave`、`setCamera`、`setMicrophone` 替换点。生产接入时可新增
`AgoraRTCAdapter`，凭证由后端按房间权限短期签发，不把 App Certificate/Secret 写入前端。
