...
const exactTranslations: Record<string, string> = {
  ...
  "阅读": "Reading",
  "数学": "Math",
  "耐心陪伴每一次进步": "Patiently supporting every step of progress",
  "星光一组": "Starlight Group 1",
  "MVP 演示小组": "MVP Demo Group",
  "快乐阅读伴学课": "Happy Reading Companion Class",
  "晨读 15 分钟": "15-minute Morning Reading",
  "大声朗读今天喜欢的故事": "Read today's favorite story aloud",
  "完成数学练习": "Complete Math Practice",
  "认真检查每一道题": "Carefully check every problem",
  "整理错题本": "Organize Mistake Notebook",
  "把今天的新发现记下来": "Write down today's new discoveries",
  "老师": "Teacher",
  "选中学生": "Selected Student",
  "实时白板、课件同步、正向奖励与温和专注提醒": "Live whiteboard, courseware sync, positive rewards, and gentle focus reminders",
  "我是学生": "I am a Student",
  "进入温馨、有趣的伴学空间": "Enter a warm and fun companion learning space",
  "打开学生端 →": "Open Student App →",
  "我是教师": "I am a Teacher",
  "创建课堂并进行实时互动教学": "Create classes and teach interactively in real time",
  "打开教师端 →": "Open Teacher App →",
  "平台管理": "Platform Admin",
  "管理账号、分组与课堂记录": "Manage accounts, groups, and class records",
  "打开管理后台 →": "Open Admin Console →",
  "演示环境仅用于产品体验，请勿录入真实儿童敏感信息。": "This demo is for product experience only. Do not enter real children's sensitive information.",
  ...
  "彩带": "Confetti",
  "星星雨": "Star Rain",
  "你真棒！继续加油！": "Great job! Keep going!",
  "太出色啦！这是你的奖杯！": "Excellent! This trophy is for you!",
  "为认真学习的你喝彩！": "Cheers for your careful learning!",
  "每一颗星星都为你闪亮！": "Every star is shining for you!",
  "继续加油！": "Keep going!",
  ...
  "AI 陪练助手": "AI Practice Buddy",
  "可以陪你背单词、练口算、复述绘本、讲错题，也可以回答学习问题。": "It can help you practice vocabulary, mental math, story retelling, mistake review, and learning questions.",
  "AI 不会读取你的摄像头或麦克风；只会发送你输入的文字和必要学习记录。不要输入电话、地址、密码或家长联系方式。": "AI does not read your camera or microphone; it only sends your typed text and necessary learning records. Do not enter phone numbers, addresses, passwords, or parent contact details.",
  "选择练习方式": "Choose Practice Mode",
  "背单词": "Vocabulary",
  "输入一个单词，比如 apple": "Type a word, e.g. apple",
  "练口算": "Mental Math",
  "输入一道口算题，比如 18 + 7": "Type a mental math question, e.g. 18 + 7",
  "复述绘本": "Story Retelling",
  "输入你刚读完的一句话或故事内容": "Type a sentence or story you just read",
  "错题问答": "Mistake Review",
  "输入错题或不懂的步骤": "Type the problem or the step you do not understand",
  "问问题": "Ask a Question",
  "输入一个学习问题": "Type a learning question",
  "儿童安全模式": "Child-safe mode",
  "把你想练习的内容发给 AI 小助手。": "Send what you want to practice to the AI buddy.",
  "我": "Me",
  "真实 AI 陪练": "Real AI Practice",
  "安全兜底回复": "Safe fallback reply",
  "发送失败": "Failed to send",
  "AI 陪练暂时不可用，请稍后再试。": "AI Practice is temporarily unavailable. Please try again later.",
  "思考中…": "Thinking…",
  "发送": "Send",
  "真实 AI 陪练暂时不可用，已返回安全兜底回复。": "Real AI practice is temporarily unavailable. A safe fallback reply was returned.",
  "未配置 OPENAI_API_KEY，AI 陪练暂未启用。": "OPENAI_API_KEY is not configured. AI Practice is not enabled yet.",
  "请选择正确的 AI 陪练模式": "Please choose a valid AI practice mode.",
  "请输入要练习的内容": "Please enter what you want to practice.",
  "单次提问请控制在 800 字以内": "Please keep each question under 800 characters.",
  ...
  "小眼睛看回来啦，我们继续专心学习哦！": "Eyes back here. Let's keep focusing.",
  ...
  "AI 陪练日志": "AI Practice Logs",
  "仅记录模式、成功状态和长度，不保存学生原文": "Only mode, success status, and length are recorded. Student text is not stored.",
  "模式": "Mode",
  "来源": "Source",
  "长度": "Length",
  "管理员": "Admin",
...
const patternTranslations: Array<[RegExp, (match: RegExpMatchArray) => string]> = [
  ...
  [/^(\d+) \/ (\d+)$/, (m) => `${m[1]} / ${m[2]}`],
  [/^(.+)的伴学课$/, (m) => `${translateDynamicText(m[1] ?? "")}'s companion class`],
  [/^(.+)的成长宝箱$/, (m) => `${translateDynamicText(m[1] ?? "")}'s Growth Chest`],
  [/^(.+)，今天也要\s*元气满满地学习哦！$/, (m) => `${translateDynamicText(m[1] ?? "")}, let's learn with full energy today!`],
  [/^(.+) 完成任务啦！$/, (m) => `${translateDynamicText(m[1] ?? "")} completed a task!`],
  [/^《(.+)》$/, (m) => `"${translateDynamicText(m[1] ?? "")}"`],
  [/^已向\s*(.+)\s*发送奖励$/, (m) => `Reward sent to ${translateDynamicText(m[1] ?? "")}`],
  [/^已提醒\s*(.+)\s*专注$/, (m) => `Focus reminder sent to ${translateDynamicText(m[1] ?? "")}`],
  [/^已全班表扬\s*(.+)$/, (m) => `Praised ${translateDynamicText(m[1] ?? "")} in class`],
  [/^学生发送了\s*(.+)$/, (m) => `Student sent ${m[1]}`],
  [/^(.+)认真完成了学习任务，大家一起给TA鼓掌！$/, (m) => `${translateDynamicText(m[1] ?? "")} finished the learning task carefully. Let's applaud!`],
  [/^(.+)正在陪伴你$/, (m) => `${translateDynamicText(m[1] ?? "")} is with you`],
  [/^(.+)已确认完成$/, (m) => `${translateDynamicText(m[1] ?? "")} confirmed completion`],
  [/^等待(.+)确认$/, (m) => `Waiting for ${translateDynamicText(m[1] ?? "")} to confirm`],
